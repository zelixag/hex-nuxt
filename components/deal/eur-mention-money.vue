...
<template>
  <div v-if="withdrawType1 === 'eur_imove'" class="mention-money_info">
    ...
    <div class="auth-content_group"
      ...
      <span v-if="total_amount_verfiy_imove == 1" style="color: red;margin-left: 10px;">{{$t('wallet.EURimoveMixWithdraw')}}</span>
      <span v-else-if="total_amount_verfiy_imove == 2" style="color: red;margin-left: 10px;">{{$t('wallet.EURimoveMaxWithdraw')}}</span>
      <span v-else-if="total_amount_verfiy_imove == 3" style="color: red;margin-left: 10px;">{{$t('wallet.EURimoveMixFointsWithdraw')}}</span>
    ...
    <div class="auth-content_group auth-content_group_account clearfix">
      <span
        class="left">{{$t("wallet.serviceCharge")}}：{{imove_fee}} {{currencyAll.currency.currencyname.toUpperCase()}}</span>
         <!-- <span class="left pl120">USDT = {{imove_exchange_rate}} USD</span> -->
      <span class="right">{{$t("wallet.actual")}}：{{imove_amount}} EUR</span>
    </div>
  ...
  export default {
    ...
    watch: {
      'mentionCoin.amount': function () {
        ...
        if(this.withdrawType1 == 'eur_imove'){
          // this.imove_exchange_rate = this.$store.getters.get_exchange_rate_by_name('usdt','usd')
          if(this.mentionCoin.amount < 50){
            ...
            var is_lttwofoints_imove = this.mentionCoin.amount.match(/^\d+(\.\d{1,2})?$/)
          ...
          this.currencyAll.currency.withdrawminfee = this.global_get_tofixed(this.mentionCoin.amount*0.0085,2)
          this.imove_fee = this.currencyAll.currency.withdrawminfee
          this.imove_amount = this.global_get_tofixed(this.$np.minus(this.mentionCoin.amount, this.currencyAll.currency.withdrawminfee),2)
        }else{
    ...
    methods: {
      ...
      confirMention() {
        ...
        if (this.currencyAll.currency.currencyname.toLowerCase() === 'eur') {
          if (this.withdrawType1 == 'eur_imove') {
            coin.ctype = 'imove'
