  ...
  <div class="exchange-block_content clearfix exchange-block_profit-loss">
    <div class="exchange-block_content_left left">
      ...
      <p
        ...
        <span class="right">
          ...
          {{openType == 1 ? $t('deal.usable') : $t('contract.flatZh')}}<span>{{getBuyUseAmount}} {{openType == 1 ?symblefrom:"USDT"}}</span></span>
      </p>
      <div class="exchange-block_button_buy"
           v-loading="profitLossBuyState"
           v-if="$userinfo.uid"
           @click="profitLossBuyDeal"
           :class="{'exchange-block_button--unavailable':(profitLossBuyState)}">{{openType===1 ?$t("memberCenter.buycontract"):$t("memberCenter.buyopencontract")}}
      </div>
    ...
    <div class="exchange-block_content_right right">
      ...
      <span class="right">
        ...
        </div>{{openType == 1 ? $t('deal.usable') : $t('contract.flatZh')}}
        <span >{{getSaleUseAmount}} {{openType == 1 ?symblefrom:"USDT"}}</span></span>
      </p> 
  ...
  computed: {
    ...
    buyable: function() {
      const a = this.global_get_tofixed(
        ...
        this.global_get_decimal(this.symbleto).a
      );
      const num = this.$store.getters.get_to_from_exchange_rate(
        a,
        this.copeEntrustBuyRate
      );
      return num;
    ...
    buyAvailable: function() {
      const a = this.global_get_tofixed(
        ...
        this.global_get_decimal(this.symblefrom).a
      );
      return this.$store.getters.get_from_to_exchange_rate(
        a,
        this.copeEntrustSaleRate
      );
    }
