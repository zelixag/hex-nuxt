  ...
  <div class="exchange-record">
    <div class="exchange-record_title">
      <span
        ...
        @click="tabChange(1)">{{$t('contract.positions')}}</span>
      <span
        :class="{'active':changeState==2}"
        @click="tabChange(2)">{{$t('patch.OpenOrder')}} </span>
      <span
         v-show="contractPage"
        :class="{'active':changeState==3}"
        @click="tabChange(3)"> {{$t('patch.TradeHistory')}}</span>
      <span
         v-show="contractPage"
        :class="{'active':changeState==4}"
        @click="tabChange(4)">{{$t('contract.financialRecords')}}</span>
...
<script>
  import entrust from '@/components/deal/order-list-entrust-contract'
  import positions from '@/components/deal/order-list-positions'
  import transaction from '@/components/deal/order-list-transaction-contract'
