      ...
      <div class="traded_table_content">
        <table class="hex_table abc">
          ...
          <tr class="hex_table_body padding"
            ...
            <td class="rt">
              {{global_get_tofixed(global_get_fee_contract(coins).toFixed(9).replace(/\.?0+$/, ""),global_get_decimal(coins.symble).p)}}
              <el-tooltip v-if="coins.licofee > 0" placement="top">
                  <div slot="content">{{$t('legalTrad.licofee')}}: {{global_get_tofixed(coins.licofee,global_get_decimal(coins.symble).p)}}</div>
