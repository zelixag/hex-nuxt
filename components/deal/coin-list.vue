      ...
      <div v-bar
        ...
        <div>
          <div class="body clearfix"
            ...
            <div class='market'>
              <span class='colour'>
                <span class="market_from">{{getSymble(item.symble).from}}</span>/{{getSymble(item.symble).to}}</span>
            </div>
            <div class="price">
  ...
  export default {
    ...
    sockets() {
      return {
        ...
        /*订阅轮询*/
        polling: {
          url: this.$socket.url.quotation_daily_getall,
    ...
    methods: {
      ...
      gettitlelist() {
        ...
        this.$store.dispatch(this.$socket.url.quotation_daily_getall, {ctype: window.localStorage.getItem('thisMarket'),iscontract : 0})
          .then(({data}) => {
            if (data) {
              data.forEach((_d) => {
                this.iscollectList[_d.ctid] = _d.self
      ...
      goDeal(coinid) {
        ...
        this.$store.commit('set_local_deal_symble', coinid);
