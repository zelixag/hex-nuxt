      ...
      <div class="traded_table_content">
        <table class="hex_table">
          ...
          <tr>
            ...
            <th class="rt">
              <div>{{$t('contract.pricetype')}}</div>
            </th>
            <th class="rt">
              <div>{{$t('contract.leverage')}}</div>
            </th>
            <th class="rt">
          ...
          <tr class="hex_table_body padding"
            ...
            <td class="rt">
              {{coins.ordertype >= 4 ? "--" :(coins.multiple + "X")}}
            </td>
            <td class="rt" v-if="coins.ordertype==1||coins.ordertype==4||coins.ordertype==3||coins.ordertype==6">
    ...
    methods: {
      ...
      getUserDeal() {
        if (this.$userinfo.uid) {
          ...
          /*清除定时器*/
          this.$socket.cleartimer(this.$socket.url.usercontract_order_search)

          serachOrder.itype = (this.margin === 'margin') ? 1 : 0;

          this.$store.dispatch('getIntervalData', {
            url: this.$socket.url.usercontract_order_search,
            params: serachOrder,
            timer: this.$socket.timer.usercontract_order_search,
            callback: (res) => {
              const data = res.data
