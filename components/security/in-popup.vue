  ...
  export default {
    ...
    methods: {
      ...
      confirmToRepayment() {
        ...
        this.$emit("confirmToRepayment", {amount: this.repayment, currencyid: this.currencyid, itype: 1});
      },
      //设置资产类型以及资产金额
      setAssetType(val) {
        this.repayment = '';
        ...
        for (let item of this.storeCurrencyData) {
          if (item.currency.currencyname.toLowerCase() === val.toLowerCase()) {
            this.currencyid = item.currency.id;
            this.availableBalance = item.canuseamount;
      ...
      assetsBalance() {
        this.$store.dispatch('user_assets_get', {assetstype: 1, symble: this.currencyPair}).then(({data}) => {
          if (data) {
            this.storeCurrencyData = data;

            for (let item of data) {
              if (item.currency.currencyname.toLowerCase() === this.currencyChoice.toLowerCase()) {
                this.currencyid = item.currency.id;
                this.availableBalance = item.canuseamount;
